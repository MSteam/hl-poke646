	...
	for (i = 0; i < JOY_MAX_AXES; i++)
	{
		...
		if (joy_wwhack2->value != 0.0)
		{
			if (dwAxisMap[i] == AxisTurn)
			{
				...
				fTemp = 300.0 * std::pow(std::abs(fAxisValue) / 800.0, 1.3);
				if (fTemp > 14000.0)
					fTemp = 14000.0;
		...
		case AxisForward:
			if ((joy_advanced->value == 0.0) && (in_jlook.state & 1))
			{
				...
				if (std::abs(fAxisValue) > joy_pitchthreshold->value)
				{		
			...
			else
			{
				...
				if (std::abs(fAxisValue) > joy_forwardthreshold->value)
				{
		...
		case AxisSide:
			if (std::abs(fAxisValue) > joy_sidethreshold->value)
			{
		...
		case AxisTurn:
			...
			else
			{
				...
				if (std::abs(fAxisValue) > joy_yawthreshold->value)
				{
